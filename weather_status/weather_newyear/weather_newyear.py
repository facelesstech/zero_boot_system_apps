...
while True:
    screen.fill(black)
### Count down code
    lastImg = pygame.image.load('/home/pi/zero_boot_system_apps/weather_status/weather_newyear/newyear1.jpg') # Load up the photo you just took
    scalePhoto = pygame.transform.scale(lastImg, (240, 250)) # Scale to fit scr    een
    screen.blit(scalePhoto, (240,30))

    leaving_date = datetime.strptime('2018-12-31 00:00:00', '%Y-%m-%d %H:%M:%S')
    now = datetime.now()
#    stringTime = "%02d - %02d:%02d:%02d" % daysHoursMinutesSecondsFromSeconds(dateDiffInSeconds(now, leaving_date))
    ...
    textXmas1 = myFontSmall.render("New year", True, white) # Draw text
